...
| `useState` | flow | Restores the named state immediately before **this flow** runs, so it starts already logged in — no re-implementing login in every flow. Flows earlier in the same run (e.g. one with `clearState`) never see the restored session. If the state doesn't exist yet or the session has expired, the flow fails normally; rerun the `saveState` flow to (re)create it (or use `verfix run --fresh-state` to discard saved states first). IndexedDB restore (Firebase/MSAL token caches) only applies when the `useState` flow is the run's first flow. |
| `refreshState` | flow | Default `true`: after a flow that restored a state via `useState` passes, the live session is re-captured to the same name — so backends with single-use (rotating) refresh tokens never leave the file on disk stale. Set `false` to keep the saved state untouched (e.g. a flow that ends logged out). |
