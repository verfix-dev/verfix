...
export default function MetricsPage() {
  ...
  const load = async () => {
    ...
    if (mRes) { 
      ...
      setTrend(mRes.daily_trend || []); 
      setTopFailing(mRes.top_failing || []); 
    }
    setHealth(hRes);
    setLoading(false);
  };
        ...
        {health && (
          <div 
            style={{ 
              marginBottom: 20, 
              padding: '10px 16px', 
              background: health.status === 'healthy' ? 'rgba(52,211,153,0.06)' : 'rgba(251,191,36,0.06)', 
              border: `1px solid ${health.status === 'healthy' ? 'rgba(52,211,153,0.2)' : 'rgba(251,191,36,0.2)'}`, 
              borderRadius: 10, 
              display: 'flex', 
              alignItems: 'center', 
              gap: 16, 
          ...
          >
            <span style={{ fontWeight: 700, fontSize: 12, color: health.status === 'healthy' ? 'var(--accent-green)' : 'var(--accent-yellow)', textTransform: 'uppercase' }}>
              ● {health.status}
            </span>
            <HealthPill label="Redis" status={health.redis} />
            <HealthPill label="Database" status={health.database} />
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Queue depth: <b style={{ color: 'var(--text-primary)' }}>{health.queue_depth}</b>
            </span>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Active workers: <b style={{ color: 'var(--text-primary)' }}>{health.active_workers}</b>
            </span>
          </div>
        )}
